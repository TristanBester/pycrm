...
By changing the labelling function, we can modify the task without changing the environment or the reward machine. This modularity makes the framework highly flexible.

### 3. Semantic Mapping

It provides a semantic mapping between environment states and symbols that have meaning in the context of the task.
...
- Explore how the [Counting Reward Machine](/docs/worked-examples/letter-env/crm) defines rewards based on these symbolic events
- See how everything comes together in the [Cross-Product Environment](/docs/worked-examples/letter-env/cross-product)
- Learn about [Counterfactual Q-Learning](/docs/worked-examples/letter-env/counterfactual-q-learning) for efficient learning with CRMs
