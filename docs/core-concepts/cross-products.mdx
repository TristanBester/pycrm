...
The `CrossProduct` class provides default implementations of `_get_obs` and `to_ground_obs` that work well for continuous and deep RL settings.
...
You can override these methods for custom observation encodings. For example, in tabular settings you may prefer to use the raw integer machine state instead of a one-hot encoding:

```python
def _get_obs(
    self, ground_obs: np.ndarray, u: int, c: tuple[int, ...]
) -> np.ndarray:
    """Custom observation with raw integer machine state."""
    return np.array([ground_obs[0], ground_obs[1], u, c[0]])
...
Here's an example from the Letter World environment that overrides the default observation methods to use a tabular encoding (raw integer `u` rather than one-hot):
...
1. **State Augmentation**: The default `_get_obs` (one-hot `u` + raw counters) works well for most cases. Override it only when you need a custom encoding (e.g. tabular settings)
