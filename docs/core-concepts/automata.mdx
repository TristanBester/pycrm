...
Reinforcement learning agents typically learn from simple reward signals that are functions of the current state or state-action pair. While effective for basic tasks, this approach struggles with temporally extended goals that depend on sequences of events or require counting.
...
- Separate task specification (what to do) from environment dynamics (how to do it)
...
Vanilla Reward Machines define rewards based on sequences of symbolic events, without counting. They consist of:
...
These are equivalent to classical finite-state machines where transitions are triggered by symbolic events and produce rewards.

### Counting Reward Machines (CRMs)

Counting Reward Machines extend Vanilla RMs with counter variables that can track occurrences of events. They add:
...
This extension significantly increases the expressive power, allowing for tasks that require counting or balancing events (e.g., collecting exactly N objects, or ensuring equal numbers of different actions).
...
However, the CRM framework still requires the full syntax (`"EVENT / (-)"`) for backward compatibility.

## Creating a Counting Reward Machine

Counting Reward Machines are used for tasks that require tracking occurrences of events. Here's an example that counts and balances events:
...
5. **Terminal States**: Use `-1` to indicate task completion
...
Reward Machines provide a powerful and flexible framework for specifying complex tasks in reinforcement learning:

- **Vanilla Reward Machines** define tasks based on sequences of events
- **Counting Reward Machines** add counters to track occurrences of events
...
By using Reward Machines, you can create agents that learn to solve tasks requiring memory, counting, and complex sequential behavior in a structured, interpretable way.
