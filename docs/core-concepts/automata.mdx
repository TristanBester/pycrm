...
Counting Reward Machines use a specific syntax for transition conditions:

<Info>
    Counting Reward Machine expressions must include the counter-condition separator.
    Reward Machines use the same event formula syntax without counters; internally,
    the RM adapter converts them into CRM expressions.
</Info>
...
The event formula `φ` is a propositional formula over the environment event enum.
Event names are case-sensitive and Boolean operators are case-insensitive:

- Single event: `"A / (-)"`
- Logical AND: `"A and B / (-)"` (both A and B must occur)
- Logical OR: `"A or B / (-)"` (either A or B must occur)
- Logical NOT: `"not A / (-)"` (A must not occur)
- Tautology/default: `"/ (-)"` or `"TAU / (-)"` (only the counter condition matters)
- Complex expressions:
...
Use explicit Boolean operators between event formulas. For example, write
`"A and not B / (-)"`; shorthand such as `"A NOT B / (-)"` is not valid.

### Counter Conditions

For Counting Reward Machines, the counter-state vector `ω` specifies zero-test
requirements for each counter. The vector length must match the machine counter
