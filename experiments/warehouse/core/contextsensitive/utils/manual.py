...
def resolve_current_waypoint(u: int) -> np.ndarray:
    ...
    match u:
        ...
        case 5:
            return cw.ABOVE_BLUE
        case 4 | 3:
            return cw.GRASP_BLUE
        case 2 | 1:
            return cw.RELEASE_BLUE
        case _:
...
def resolve_gripper_state(u: int) -> float:
    ...
    match u:
        case 0 | 2 | 3 | 7 | 8 | 12 | 13:
            return -1.0
        case 4 | 5 | 1 | 10 | 9 | 6 | 15 | 14 | 11:
            return 1.0
        case _:
            raise ValueError(f"Invalid state {u}")
...
def main() -> None:
    ...
    env = gym.make(
        ...
        crossproduct_kwargs={"max_steps": 10000, "memory_scale": 100000.0},
    )
    obs, _ = env.reset()
    ...
    for _ in range(10000):
        ...
        elif last_c[2] != env.c[2]:  # type: ignore
            env.ground_env.task.scene_manager.animate_blue_block()  # type: ignore
