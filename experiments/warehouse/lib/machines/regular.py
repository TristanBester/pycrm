...
class RegularCRM(CountingRewardMachine):
    """RegularCRM for the warehouse environment."""

    def __init__(self, c_0: tuple[int] = (3,)) -> None:
        """Initialise the CRM."""
    ...
    def _get_state_transition_function(self) -> dict:
        ...
        red_block_module_delta_u = self._add_block_module_delta_u(
            u_start=1, counter_state="(NZ)", block_colour="RED"
        )
        # Merge delta_u
    ...
    def sample_counter_configurations(self) -> list[tuple[int, ...]]:
        ...
        main_configurations = list(product(range(3), repeat=1))
        random_configurations = np.random.randint(low=0, high=100_000, size=(15, 1))
    ...
    def _add_block_module_delta_c(
        ...
        module_terminal_counter_modifier = (-1,)
        base_u = u_start + 4
