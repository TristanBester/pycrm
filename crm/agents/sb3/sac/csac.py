...
#                 replay_buffer, buffer_actions, new_obs, actions
#             )
...
class CounterfactualSAC(SAC):
    def learn(
        ...
        """Override to chnage algorithm Tensorboard log name."""
        return super().learn(
            total_timesteps=total_timesteps,
            callback=callback,
            log_interval=log_interval,
    ...
    def collect_rollouts(
        ...
        # FIXME: Refactor docstring to use google style
        """Collect experiences and store them into a ``ReplayBuffer``.
        ...
        :param train_freq: How much experience to collect
            ...
            Either ``TrainFreq(<n>, TrainFrequencyUnit.STEP)``
            or ``TrainFreq(<n>, TrainFrequencyUnit.EPISODE)``
            with re``<n>`` being an integer greater than 0.
    ...
    # FIXME: This function signature seems wack, check if the parameters make sense
    def _store_counterfactual_transitions(
        self, replay_buffer: ReplayBuffer, buffer_actions, obs_next, actions
    ):
        ground_obs = self._to_ground_obs(self._last_obs)
        ground_obs_next = self._to_ground_obs(obs_next)

        dispatched_result = self.env.dispatched_env_method(
            "generate_counterfactual_experience",
            ground_obs,
            buffer_actions,
            ground_obs_next,
        ...
        # Reshape & batch to match number of envs
        c_obs = self.reshape_and_trim(
            c_obs, final_dim=self.env.observation_space.shape[0]
        )
        c_actions = self.reshape_and_trim(
            c_actions, final_dim=self.env.action_space.shape[0]
        )
        c_obs_next = self.reshape_and_trim(
            c_obs_next, final_dim=self.env.observation_space.shape[0]
        )
        c_rewards = self.reshape_and_trim(c_rewards, final_dim=1)
        c_dones = self.reshape_and_trim(c_dones, final_dim=1)
        c_infos = self.reshape_and_trim(c_infos, final_dim=1)
    ...
    # FIXME: this should not be here (as couples this implementation to that of the ground env implementation
    # which makes no sense).
    def _to_ground_obs(self, obs):
        # Remove counter machine and counter states
        return obs[:, :7]
