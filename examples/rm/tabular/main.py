...
from crm.agents.tabular import CounterfactualQLearningAgent, QLearningAgent
...
def main():
    ...
    smoothed_returns_ql = np.convolve(all_returns_ql, np.ones((10,)) / 10, mode="valid")
    smoothed_returns_cql = np.convolve(
        all_returns_cql, np.ones((10,)) / 10, mode="valid"
    )
    ...
    # Save visualization
    plt.savefig("office_world_experiment.png", dpi=300)
    plt.show()
