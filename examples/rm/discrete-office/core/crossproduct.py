...
class OfficeWorldCrossProduct(CrossProduct[np.ndarray, np.ndarray, int, None]):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=100, shape=(7,), dtype=np.int32
        )
    ...
    def _get_obs(
        ...
        """
        encoded_machine_state = self.crm.encode_machine_state(u)
        encoded_counter_configuration = self.crm.encode_counter_configuration(c)
        return np.concatenate(
            [ground_obs, encoded_machine_state, encoded_counter_configuration]
        )
